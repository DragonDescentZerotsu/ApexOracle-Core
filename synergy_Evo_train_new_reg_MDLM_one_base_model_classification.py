...
if __name__=='__main__':
    ...
    model_save_dir = current_folder / 'Checkpoints' / 'genome_text_learnable_emb' / f'strain_wise_synergy' / f'MDLM_3_fold_ensembles_1_base_model_cls'
    ...
    # dataset = SMILESDataset(data, tokenizer)
        ...
        synergy_mol_emb_dict = torch.load(current_folder / 'DataPrepare' / 'Data' / 'synergy_mol_emb_dict_cls_wo_pad.pt')
            ...
            for epoch in tqdm(range(num_epochs), desc=f' Training ensemble {ensemble + 1}/{num_ensembles} ', leave=False):
                ...
                if epoch == 0:
                    with torch.no_grad():
                        ...
                        #     auroc_species = roc_auc_score(species_wise_test_labels_dict[_speceis_name], species_wise_test_preds_dict[_speceis_name])
                        ...
                        #     r2_MSE_spearman_pearson_species_wise[_speceis_name] = [auroc_species, MSE_specise, spearman_species, pearson_species]
                        #
                        # logger.info(f'\n Test species wise AUROC, MSE, Spearman, Pearson:')
 ...
 Test Loss: {np.array(test_batch_losses).mean():.6f}, genome text Test Loss: {np.array(gt_test_batch_losses).mean():.6f}, text only Test Loss: {np.array(t_test_batch_losses).mean():.6f}
 Test AUROC: {auroc:.6f}, genome text Test AUROC: {gt_auroc:.6f}, text only Test AUROC: {t_auroc:.6f}, Test train mean MIC AUROC: {auroc_train_mean:.6f}""")
        ...
        Train AUROC: {auroc_train:.4f}, Test AUROC: {auroc_test:.4f}, Best Test AUROC: {best_auroc_test:.4f}
          ...
          Text only Train AUROC: {t_auroc_train:.4f}, Text only Test AUROC: {t_auroc_test:.4f}
        Train AUPRC:{auprc_train:.4f}, Test AUPRC:{auprc_test:.4f}, Best Test AUPRC:{best_auprc_test:.4f}
          ...
          Text only Train AUPRC:{t_auprc_train:.4f}, Text only Test AUPRC:{t_auprc_test:.4f}""")
