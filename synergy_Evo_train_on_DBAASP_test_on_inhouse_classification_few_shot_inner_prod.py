...
if __name__=='__main__':
    ...
    model_save_dir = current_folder / 'Checkpoints' / 'genome_text_learnable_emb' / f'strain_wise_synergy' / f'MDLM_train_on_DBAASP_test_on_inhouse_cls_few_shot_inner_prod'
    ...
    # dataset = SMILESDataset(data, tokenizer)
        ...
        base_model_dir = current_folder / 'Checkpoints' / 'genome_text_learnable_emb' / 'guidance_regressor_pad_no_mask' / 'noise_guidance_best_R2_all_peptide_epoch_100.pth'
        # base_model_dir = current_folder / 'Checkpoints' / 'genome_text_learnable_emb' / 'all_AMP_SM_data_train'/'MDLM_MTR_fix_cls_wo_pad' / 'all_data_best_R2_epoch_13.pth'  # 这个权重在所有的 MIC data 上训练过
                            ...
                            for gt_batch in tqdm(
                                ...
                                if gt_batch is not None:
                                    ...
                                    with torch.amp.autocast('cuda', enabled=True):
                                        ...
                                        # -----------------------------------------------------------------------
                                        mol_cls_embedding_lower_dim = reg_head(mol_cls_embedding)
                                        logits = torch.sum(mol_cls_embedding[::2] * mol_cls_embedding[1::2], dim=1)
                                        # -----------------------------------------------------------------------


                                        # loss = criterion(logits.squeeze(), labels.squeeze())
                                        loss = sigmoid_focal_loss(
                                            logits.squeeze(),  # 模型输出的 logits 张量
                                            labels.squeeze(),  # 二分类/多标签场景下的 one-hot 形式标签张量（0/1）
                                            alpha=0.7,  # 正样本与负样本的平衡因子，默认 0.25
                                            gamma=2.0,  # 聚焦因子，默认 2.0
                                            reduction='mean'  # 损失的 reduction 方式，可选 'none' | 'mean' | 'sum'
                                        )
