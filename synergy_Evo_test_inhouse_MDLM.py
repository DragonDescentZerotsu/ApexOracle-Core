...
class SMILESDataset_with_genome_and_text(Dataset):
    def __init__(self, dataframe, tokenizer, embeddings_dict, text_embeddings_dict, set_desc: str, mol_emb_dict,
                 max_length=512):
...
if __name__ == '__main__':
    ...
    model_save_dir = current_folder / 'Checkpoints' / 'genome_text_learnable_emb' / f'strain_wise_synergy' / f'inhouse_synergy_test'
    ...
    # 循环测试所有的 group
    for i, (strain_for_test, test_group_name) in enumerate(zip(test_groups, group_names)):
        ...
        gt_test_mean_MIC = -np.log10(gt_test_data['FICI'].mean() / 10)
        ...
        synergy_mol_emb_dict = torch.load(
            current_folder / 'DataPrepare' / 'Data' / 'synergy_inhouse_mol_emb_dict_cls_wo_pad.pt')
        ...
        base_model_dir = current_folder / 'Checkpoints' / 'genome_text_learnable_emb' / 'strain_wise_synergy' / 'MDLM_3_fold_ensembles_1_base_model_all_data'
        base_models = [file for file in base_model_dir.iterdir() if '.ckpt' in file.name]

        test_predictions_of_ensembles = []
        for ensemble, base_model in tqdm(enumerate(base_models), desc=' Doing ensembles '):

            logging.info(f'\n Model loaded: {base_model.name}')
            ...
            state_dict = torch.load(base_model, map_location=torch.device('cpu'), weights_only=False)
            ...
            genome_dim = gt_test_dataset[0]['genome_embedding'].shape[1]
            text_dim = gt_test_dataset[0]['text_embedding'].shape[1]

            co_cross_attn_genome = FirstTokenAttention_genome(768, gt_test_dataset[0]['genome_embedding'].shape[1], 4,
                                                              0.1)
            # co_cross_attn_genome.load_state_dict(state_dict['co_cross_attn_genome'])  # TODO: 这里在正式代码中不要注释掉

            co_cross_attn_text = FirstTokenAttention_genome(768, gt_test_dataset[0]['text_embedding'].shape[1], 4, 0.1)
                    ...
                    with torch.no_grad():
                        ...
                        for gt_batch, t_batch in tqdm(
                            ...
                            if gt_batch is not None:
                                ...
                                test_mean_as_test_predict.extend(
                                    np.full(logits.detach().cpu().flatten().shape, gt_test_mean_MIC).tolist())
                            ...
                            if t_batch is not None:
                                ...
                                test_mean_as_test_predict.extend(np.full(logits.detach().cpu().flatten().shape, t_test_mean_MIC).tolist())
                        ...
                        r2_train_mean = calculate_r2(test_all_labels, test_mean_as_test_predict)
...
Test R2: {r2:.6f}, genome text Test R2: {gt_r2:.6f}, text only Test R2: {t_r2:.6f}, Test train mean MIC R2: {r2_train_mean:.6f}""")
                        ...
                        test_predictions_of_ensembles.append(test_all_preds)
        ...
        logger.info(f' Ensemble pearson of {args.test_group}: {ensembled_pearson:.4f}')
                ...
                #             mol_cls_embedding = torch.cat(
                #                 (mol_cls_embedding_genome.reshape(-1, 8192), mol_cls_embedding_text.reshape(-1, 4096)),
                #                 dim=1)
                #             # logits = reg_head(features = mol_cls_embedding)
                #             FICI_input_1 = torch.cat((mol_cls_embedding[::2], mol_cls_embedding[1::2]), dim=1)
                #             FICI_input_2 = torch.cat((mol_cls_embedding[1::2], mol_cls_embedding[::2]), dim=1)
                ...
                #     else:
                #         spearman_species = pearson_species = None
                #     r2_MSE_spearman_pearson_species_wise[_speceis_name] = [r2_species, MSE_specise, spearman_species,
                #                                                            pearson_species]
                #
