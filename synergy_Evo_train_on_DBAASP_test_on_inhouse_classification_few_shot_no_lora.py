    ...
    # dataset = SMILESDataset(data, tokenizer)
        ...
        for fold, (train_idx, test_idx) in enumerate(kf.split(gt_test_dataset)):
            ...
            for ensemble in tqdm(range(num_ensembles), desc=' Doing ensembles '):
                ...
                reg_head = RegressionHead((genome_dim + text_dim) // 2, 128*2, 128, 1, 0.2)
                            ...
                            for gt_batch in tqdm(
                                ...
                                if gt_batch is not None:
                                    ...
                                    with torch.amp.autocast('cuda', enabled=True):
                                        ...
                                        _, MIC_input_mid = MIC_reg_head(mol_cls_embedding)
                                        # _, MIC_input_1_mid = MIC_reg_head(mol_cls_embedding[1::2])
                                        FICI_input_1 = torch.cat((MIC_input_mid[::2], MIC_input_mid[1::2]), dim=1)
                                        FICI_input_2 = torch.cat((MIC_input_mid[1::2], MIC_input_mid[::2]), dim=1)
                                        ...
                                        logits_1, _ = reg_head(FICI_input_1)
                                        logits_2, _ = reg_head(FICI_input_2)
