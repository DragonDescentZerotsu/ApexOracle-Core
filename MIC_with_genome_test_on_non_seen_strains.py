...
group_names = ['fold 1', 'fold 2', 'fold 3', '1 strain species']  # 6, 9, 10, 12, 13, 14, 30, 31, 32, 33, 41 缩写字母代表
...
for i, (strain_for_train, strain_for_test, test_group_name) in enumerate(zip(train_groups, test_groups, group_names)):
    ...
    for ensemble in tqdm(range(num_ensembles), desc=' Doing ensembles '):
        ...
        optimizer.add_param_group({'params': ChemBERTa_model.parameters(), 'lr': 7e-7})   # TODO: ChemBERTa 和别的学习率不一样
        ...
        for epoch in tqdm(range(num_epochs), desc=f' Training ensemble {ensemble+1}/{num_ensembles} ', leave=False):
            ...
            with torch.no_grad():
                ...
                if r2_test > best_R2_test:
                    ...
                    }, current_directory / 'Checkpoints' / 'genome' / '11_fold_ensembles' / f'Strain_wise_best_R2_{args.test_group}_ensemble_{ensemble}.pth')
