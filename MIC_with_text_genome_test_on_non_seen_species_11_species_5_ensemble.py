...
def load_all_genome_embeddings(embeddings_folder_path, scale, device, desc_str):
    ...
    embeddings_dict = {}
    for file_path in tqdm(file_paths, desc=f' loading {desc_str} embeddings ... ', disable=not sys.stdout.isatty()):
...
def cluster_species(path_to_phy_tree:Path, old_to_new_NCBI_taxonomy_map_path:Path, num_clusters:int=5):
    ...
    for i in tqdm(range(n), desc=' computing distance matrix', disable=not sys.stdout.isatty()):
        for j in range(i, n):
...
model_save_dir = current_directory / 'Checkpoints' / 'genome_text' / '11_species' / f'{num_ensembles}_fold_ensembles'
...
Evo_MIC_count_file_path = current_directory / 'DataPrepare' / 'Data' / 'Evo_edition_3_MIC_data_handcrafted_no_ATCC_to_custom_ATCC.json'
...
# 去掉那些带 'del' 的
del_excluded_data = []
for MIC_data_line in tqdm(all_Evo_MIC_data, desc=' removing MIC data with "del" in name ', disable=not sys.stdout.isatty()):
...
for MIC_data_line in tqdm(all_Evo_MIC_data, desc=' retriving MIC data with genome embeddings ', disable=not sys.stdout.isatty()):
...
print(f'\n loading text data...')
text_embeddings_dict = load_all_genome_embeddings(text_embeddings_folder_path, text_embedding_scale_factor, device, 'text')
...
for standard_strain_ID in tqdm(all_standard_name_set, desc=' Getting strain MIC groups ', disable=not sys.stdout.isatty()):
...
all_grouped_species = cluster_species(current_directory/'DataPrepare'/'Data'/'Genome'/'visualization'/'Serratia_replaced_tree_Fungi_Enterobacterales.phy',
                                      current_directory/'DataPrepare'/'Data'/'Genome'/'old_to_new_NCBI_taxonomy_ablation.json',
...
all_grouped_strains = []
for grouped_species in tqdm(all_grouped_species, desc=' Grouping strains by clustered species', disable=not sys.stdout.isatty()):
    grouped_strains = []
    for species in grouped_species:
        if species == 'Candida guilliermondii':
            species = 'Meyerozyma guilliermondii'
        grouped_strains.extend(species_name_ATCC_IDs_map_dict[species])
...
# 排序的依据是每个元组中第一个元素（即 species）的长度，排序规则是按长度从小到大排序
...
# 循环测试所有的 group
for i, (grouped_species, grouped_strains, test_group_name) in enumerate(zip(all_grouped_species, all_grouped_strains, group_names)):
    ...
    strain_for_test = set(grouped_strains) & all_standard_name_set
    ...
    train_mean_MIC = -np.log10(train_data['MIC'].mean()/10)

    model_name = "DeepChem/ChemBERTa-77M-MTR"
    tokenizer = AutoTokenizer.from_pretrained(model_name)

    train_dataset = SMILESDataset_with_genome_and_text(train_data, tokenizer, embeddings_dict, text_embeddings_dict, 'training set')
    test_dataset = SMILESDataset_with_genome_and_text(test_data, tokenizer, embeddings_dict, text_embeddings_dict, 'test set')
    ...
    print(f'\n start training emsembles')
    for ensemble in tqdm(range(num_ensembles), desc=' Doing ensembles ', disable=not sys.stdout.isatty()):
        # 设置对应的 随机数种子
        ...
        genome_dim = train_dataset[0]['genome_embedding'].shape[1]
        text_dim = train_dataset[0]['text_embedding'].shape[1]
        ...
        co_cross_attn_text = FirstTokenAttention_genome(ChemBERTa_model.config.hidden_size, train_dataset[0]['text_embedding'].shape[1], 4, 0.1)
        ...
        optimizer.add_param_group({'params': ChemBERTa_model.parameters(), 'lr': 6e-6, 'weight_decay': args.weight_decay*0.1})   # TODO: ChemBERTa 和别的学习率不一样

        scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=num_epochs, eta_min=min_lr)

        train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn)#, num_workers=args.num_workers)
        test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn)#, num_workers=args.num_workers)
        ...
        print(f'\n starting training ensenmle {ensemble+1}')
        for epoch in tqdm(range(num_epochs), desc=f' Training ensemble {ensemble+1}/{num_ensembles} ', leave=False, disable=not sys.stdout.isatty()):
            ...
            if epoch == 0:
                with torch.no_grad():
                    ...
                    for batch in tqdm(test_loader, desc=f" Epoch {epoch}/{num_epochs} | evaluating", leave=False, disable=not sys.stdout.isatty()):
                        ...
                        train_mean_as_test_predict.extend(np.full(logits.detach().cpu().flatten().shape, train_mean_MIC).tolist())

                    r2 = calculate_r2(test_all_labels, test_all_preds)
                    r2_train_mean = calculate_r2(test_all_labels, train_mean_as_test_predict)

                    print(
                        f" Ensemble {ensemble+1}/{num_ensembles} Epoch {epoch}/{num_epochs}, Test Loss: {np.array(test_batch_losses).mean():.6f}, Test R2: {r2:.6f}, Test train mean MIC R2: {r2_train_mean:.6f}")

            train_batch_losses = []
            train_all_labels = []
            train_all_preds = []

            for batch in tqdm(train_loader, desc=f" Ensemble {ensemble+1}/{num_ensembles} Epoch {epoch + 1}/{num_epochs} | training", leave=False, disable=not sys.stdout.isatty()):
                input_ids = batch['input_ids'].to(device)
                attention_mask = batch['attention_mask'].to(device)
                labels = batch['label'].to(device)
                padded_genome_embeddings = batch['padded_genome_embeddings']  # .to(torch.float)
            ...
            with torch.no_grad():
                ...
                for batch in tqdm(test_loader, desc=f" Ensemble {ensemble+1}/{num_ensembles} Epoch {epoch + 1}/{num_epochs} | evaluating", leave=False, disable=not sys.stdout.isatty()):
                    input_ids = batch['input_ids'].to(device)
                    attention_mask = batch['attention_mask'].to(device)
                    labels = batch['label'].to(device)
                    padded_genome_embeddings = batch['padded_genome_embeddings']  # .to(torch.float)
                ...
                if r2_test > best_R2_test:
                    ...
                    }, model_save_dir / f'genome_text_Species_wise_best_R2_{args.test_group}_ensemble_{ensemble}.pth')
