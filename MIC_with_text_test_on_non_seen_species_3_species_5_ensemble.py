...
num_ensembles = 5  # 要集成几个 model 来做预测
...
for grouped_species, grouped_strains, test_group_name in zip(all_grouped_species, all_grouped_strains, group_names):
    ...
    for ensemble in tqdm(range(num_ensembles), desc=' Doing ensembles '):
        ...
        optimizer = optim.Adam(co_cross_attn_genome.parameters(), lr=1e-5)
        optimizer.add_param_group({'params': reg_head.parameters(), 'lr': 1e-5})
        optimizer.add_param_group({'params': ChemBERTa_model.parameters(), 'lr': 6e-6})
        ...
        for epoch in tqdm(range(num_epochs), desc=f' Training ensemble {ensemble+1}/{num_ensembles} ', leave=False):
            ...
            if epoch == freeze_epochs:
                ...
                # optimizer.add_param_group({'params': ChemBERTa_model.parameters(), 'lr': 1e-5})
            ...
            with torch.no_grad():
                ...
                if r2_test > best_R2_test:
                    ...
                    }, current_directory / 'Checkpoints' / 'genome' / 'ensembles' / f'text_best_R2_{args.test_group}_ensemble_{ensemble}.pth')
