...
class PeptideEmbeddings(nn.Module):
    def __init__(self, emb):
        ...
        self.aa_embedding = nn.Embedding.from_pretrained(torch.FloatTensor(emb), padding_idx=0, freeze=True)
        # self.aa_embedding = nn.Embedding(23, 384)
    def forward(self, x):
...
class AMP_model_fix(nn.Module):
    def __init__(self, emb, emb_size, num_rnn_layers, dim_h):
        super().__init__()
