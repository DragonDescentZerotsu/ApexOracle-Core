...
def format_inhouse_data(data_path):
    df = pd.read_csv(data_path)
    colume_names = list(df.columns)[:-1]
    colume_names[0] = 'AAseqs'
    ...
    inhouse_train = inhouse_data[inhouse_data[:, -1] == 'CV'][:, :-1]
    inhouse_test = inhouse_data[inhouse_data[:, -1] != 'CV'][:, :-1]
    ...
    df_train.to_csv('../DataPrepare/Data/APEX_train_peptide.csv', index=False)
    df_test.to_csv('../DataPrepare/Data/APEX_test_peptide.csv', index=False)
...
class AAseqsDataset(Dataset):
    def __init__(self, dataframe, max_length, word2vec):
        ...
        # TODO: 这个需要根据不同的数据文件改取哪几列
        self.target_columns = self.dataframe.columns.tolist()[1:]
...
def collate_fn(batch):
    ...
    # 计算实际的用来回归的值
    labels_processed[mask] = -torch.log10(labels[mask] / 1000000)
    mask = mask.int()
...
format_inhouse_data('../DataPrepare/Data/APEX 1.1 Data.csv')

train_data_path = '/home/junyuz6/cold/Synergy/DataPrepare/Data/APEX_train_peptide.csv'  # 替换为你的数据路
train_data = pd.read_csv(train_data_path)

test_data_path = '/home/junyuz6/cold/Synergy/DataPrepare/Data/APEX_test_peptide.csv'  # 替换为你的数据路
test_data = pd.read_csv(test_data_path)

train_dataset = AAseqsDataset(train_data, max_len, word2idx)
test_dataset = AAseqsDataset(test_data, max_len, word2idx)
...
print(f'Current Dataset length: {len(train_dataset)}, Original Dataset length: {train_dataset.original_length}, cutting off length: {train_dataset.max_length}')
...
wandb.init(
    # set the wandb project where this run will be logged
    project="Synergy",
    name=f'APEX_inhouse_data_{batch_size}batch_size_nonfreeze_embedding, emb size {emb_size}',
...
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn)  # TODO
test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn)  # TODO

# 加载模型
model = AMP_model(emb, emb_size, num_rnn_layers = 3, dim_h = 256, dim_latent = 256, num_fc_layers=3, num_task = 11)  # TODO
...
cosine_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, num_epochs, eta_min=1e-7)

r2_tracker = R2Tracker(num_tasks=11)  # TODO
...
for epoch in range(num_epochs):
    ...
    if R2_mean > R2_mean_best:
        R2_mean_best = R2_mean
    ...
    wandb.log({"loss": loss.item(), "R2_mean": R2_mean})
    print(
        f"Epoch {epoch + 1}/{num_epochs}, Loss: {loss.item()}\nmean R2 Score: {R2_mean}, best mean R2: {R2_mean_best}\nR2 Score per task: {R2_per_task}\nbest R2 Score: {r2_tracker.get_best_r2()}\n")
wandb.log({"best_mean_R2_across_folds": R2_mean_best})
wandb.finish()
# break
