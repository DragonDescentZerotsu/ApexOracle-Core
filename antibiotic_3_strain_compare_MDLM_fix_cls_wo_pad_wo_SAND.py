...
class SMILESDataset_with_genome_and_text(Dataset):
    def __init__(self, dataframe, tokenizer, embeddings_dict, text_embeddings_dict, set_desc:str, pep_emb_dict, SM_emb_dict, max_length=512):
...
class SMILESDataset_with_text_only(SMILESDataset_with_genome_and_text):
    def __init__(self, dataframe, tokenizer, text_embeddings_dict, set_desc: str, pep_emb_dict, SM_emb_dict, max_length=512):
        # 调用父类的 __init__ 方法时，可以将 embeddings_dict 传入一个 None 或者空字典（如果父类内部没有用到的话）
        super().__init__(dataframe, tokenizer, embeddings_dict=None, text_embeddings_dict=text_embeddings_dict, set_desc=set_desc, pep_emb_dict=pep_emb_dict, SM_emb_dict=SM_emb_dict, max_length=max_length)
...
def collate_fn(batch):
    ...
    mol_emb = [item['mol_emb'] for item in batch]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help='How many folds to test'
    )
    parser.add_argument(
        '-f', '--set_fold',  # 可选参数
        type=int,
        default=0,
        help='Which folds to test'
    )
    parser.add_argument(
        '-w', '--weight_decay',  # 可选参数
        type=float,
    ...
    num_ensembles = 10  # 要集成几个 model 来做预测
    ...
    model_save_dir = current_directory / 'Checkpoints' / 'genome_text_learnable_emb' / 'antibiotic_3_strain_compare' / f'MDLM_fix_cls_wo_sand_{num_ensembles}_fold_ensembles'
    ...
    # 创建文件Handler
    file_handler = logging.FileHandler(model_save_dir/f'log_group_{args.test_group}_fold_{args.set_fold}.log', mode='w')
    ...
    # 循环测试所有的 group
    for group_i, test_group_name in enumerate(group_names):

        # 如果要 parallel 地 validate，那么在当前 group 不是 目标 test group 的时候直接跳过
        if args.parallel:
            if group_i != args.test_group:
                continue
        ...
        gt_train_data = Evo_MIC_data_with_genome_embedding_standard_name
        ...
        gt_train_data = pd.DataFrame(gt_train_data, columns=columns_names)
        ...
        t_train_data = Evo_MIC_data_with_genome_or_text_embedding
        ...
        t_train_data = pd.DataFrame(t_train_data, columns=columns_names)
        ...
        SM_gt_strain_for_test = {test_group_name} & SM_gt_all_name_set
        # SM_gt_strain_for_train = SM_gt_all_name_set - SM_gt_strain_for_test
        SM_gt_strain_for_train = SM_gt_all_name_set - set(SM_gt_strain_for_test)
        ...
        SM_t_strain_for_test = {test_group_name} & {'Staphylococcus aureus RN4220'}
        ...
        pep_emb_dict = torch.load(current_directory / 'DataPrepare' / 'Data' / 'Pep_emb_dict_cls_wo_pad_eval.pt')
        SM_emb_dict = torch.load(current_directory / 'DataPrepare' / 'Data' / 'SM_emb_dict_cls_wo_pad_eval.pt')

        # gt_train_dataset = SMILESDataset_with_genome_and_text(gt_train_data, tokenizer, embeddings_dict, text_embeddings_dict, 'peptide genome-text training set', pep_emb_dict, SM_emb_dict)
        ...
        if len(SM_gt_train_data) > 0:
            SM_gt_train_dataset = SMILESDataset_with_genome_and_text(SM_gt_train_data, tokenizer, embeddings_dict, text_embeddings_dict, 'small-molecule genome-text training set', pep_emb_dict, SM_emb_dict)
        SM_gt_test_dataset = None
        if len(SM_gt_test_data) > 0:
            SM_gt_test_dataset = SMILESDataset_with_genome_and_text(SM_gt_test_data, tokenizer, embeddings_dict, text_embeddings_dict, 'small-molecule genome-text test set', pep_emb_dict, SM_emb_dict)
        ...
        # t_train_dataset = SMILESDataset_with_text_only(t_train_data, tokenizer, all_text_embedding_dict, 'peptide text-only training set', pep_emb_dict, SM_emb_dict)
        # t_test_dataset = SMILESDataset_with_text_only(t_test_data, tokenizer, all_text_embedding_dict,'peptide text-only test set')

        if len(SM_t_train_data) > 0:
            SM_t_train_dataset = SMILESDataset_with_text_only(SM_t_train_data, tokenizer, all_text_embedding_dict, 'small-molecule text-only training set', pep_emb_dict, SM_emb_dict)
        SM_t_test_dataset = None
        if len(SM_t_test_data) > 0:
            SM_t_test_dataset = SMILESDataset_with_text_only(SM_t_test_data, tokenizer, all_text_embedding_dict, 'small-molecule text-only test set', pep_emb_dict, SM_emb_dict)
        ...
        if SM_gt_test_dataset is not None:
            outer_test_SM_dataset = SM_gt_test_dataset
            gt_flag = True
        else:
            outer_test_SM_dataset = SM_t_test_dataset
            gt_flag = False
        # outer_test_SM_dataset = SM_gt_test_dataset if SM_gt_test_dataset is not None else SM_t_test_dataset
        if SM_gt_test_dataset is None and SM_t_test_dataset is None:
            print("No test data")
            exit(1)
        ...
        auroc_list = []
        auprc_list = []
        for fold, (train_idx, test_idx) in tqdm(enumerate(kf.split(outer_test_SM_dataset)), desc=' Folds', total=num_folds):
        # for fold in range(num_folds):
            if args.set_fold != fold:
                continue

            SM_train_subset = torch.utils.data.Subset(outer_test_SM_dataset, train_idx)
            SM_test_subset = torch.utils.data.Subset(outer_test_SM_dataset, test_idx)

            # 每一个 ensemble 都记录对 test set 的预测值然后再求平均即可
            test_predictions_of_ensembles = []
            for ensemble in tqdm(range(num_ensembles), desc=f' Doing ensembles of Fold {fold}'):
                # 设置对应的 随机数种子
                ...
                # co_cross_attn_genome = FirstTokenAttention_genome(768, gt_train_dataset[0]['genome_embedding'].shape[1], 4, 0.1)
                ...
                # reg_head.to(device)
                cls_head = RegressionHead(768, 768 // 2, 128, 1, 0.2)
                cls_head.to(device)
                ...
                optimizer = optim.Adam(cls_head.parameters(), lr=1e-5, weight_decay=args.weight_decay)  # 1e-5
                ...
                if gt_flag:
                    outer_test_SM_train_loader = DataLoader(SM_train_subset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn_cls)
                    outer_test_SM_test_loader = DataLoader(SM_test_subset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn_cls)
                else:
                    outer_test_SM_train_loader = DataLoader(SM_train_subset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn_text_only_cls)
                    outer_test_SM_test_loader = DataLoader(SM_test_subset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn_text_only_cls)

                # best_R2_test = -10
                best_auroc_test = -10
                best_auprc_test = -10
                best_test_prdictions = None
                for epoch in tqdm(range(num_epochs), desc=f' Training ensemble {ensemble+1}/{num_ensembles} of Fold {fold}', leave=False):
                    ...
                    outer_cls_train_batch_losses = []
                    outer_cls_train_all_labels = []
                    outer_cls_train_all_preds = []
                    ...
                    #         mol_cls_embedding = SM_gt_batch['mol_emb'].to(device)
                    #
                    #         optimizer.zero_grad()
                    #
                    #         with torch.amp.autocast('cuda', enabled=True):
                    ...
                    #         mol_cls_embedding = SM_t_batch['mol_emb'].to(device)
                    #
                    #         optimizer.zero_grad()
                    #
                    #         with torch.amp.autocast('cuda', enabled=True):
                    ...
                    # 对于需要在最外侧进行
                    for batch in tqdm(outer_test_SM_train_loader, desc=' Outer test cls train', leave=False):
                        # input_ids = batch['input_ids'].to(device)
                        # attention_mask = batch['attention_mask'].to(device)
                        labels = batch['label'].to(device)
                        # if gt_flag:
                        #     padded_genome_embeddings = batch['padded_genome_embeddings']  # .to(torch.float)
                        #     genome_attn_masks = batch['genome_attn_masks']
                        # padded_text_embeddings = batch['padded_text_embeddings']  # .to(torch.float)
                        # text_attn_masks = batch['text_attn_masks']
                        # strain_names = batch['strain_names']
                        mol_cls_embedding = batch['mol_emb'].to(device)
                        ...
                        outer_cls_train_batch_losses.append(loss.item())
                        outer_cls_train_all_labels.extend(labels.detach().cpu().flatten().tolist())
                        outer_cls_train_all_preds.extend(logits.detach().cpu().flatten().tolist())
                    ...
                    # TODO: 把这里加上对外面分类的 AUROC 和 AUPRC
                    y_true = np.array(outer_cls_train_all_labels)  # 二分类标签，0/1
                    y_scores = np.array(outer_cls_train_all_preds)  # 模型输出的 logits 或概率
                    outer_test_train_auroc = roc_auc_score(y_true, y_scores)
                    outer_test_train_auprc = average_precision_score(y_true, y_scores)
                    ...
                    with torch.no_grad():
                        ...
                        outer_cls_test_batch_losses = []
                        outer_cls_test_all_labels = []
                        outer_cls_test_all_preds = []

                        # co_cross_attn_genome.eval()
                        # co_cross_attn_text.eval()
                        cls_head.eval()

                        for batch in tqdm(outer_test_SM_test_loader, desc=' Outer test cls test', leave=False):
                            # input_ids = batch['input_ids'].to(device)
                            # attention_mask = batch['attention_mask'].to(device)
                            labels = batch['label'].to(device)
                            # if gt_flag:
                            ...
                            outer_cls_test_batch_losses.append(loss.item())
                            outer_cls_test_all_labels.extend(labels.detach().cpu().flatten().tolist())
                            outer_cls_test_all_preds.extend(logits.detach().cpu().flatten().tolist())
                        ...
                        # TODO: 把这里加上对外面分类的 AUROC 和 AUPRC
                        y_true = np.array(outer_cls_test_all_labels)  # 二分类标签，0/1
                        y_scores = np.array(outer_cls_test_all_preds)  # 模型输出的 logits 或概率
                        outer_test_test_auroc = roc_auc_score(y_true, y_scores)
                        outer_test_test_auprc = average_precision_score(y_true, y_scores)
                        ...
                        if outer_test_test_auroc > best_auroc_test:
                            best_auroc_test = outer_test_test_auroc
                            best_test_prdictions = outer_cls_test_all_preds

                            torch.save({
                                'auroc': best_auroc_test,
                                'auprc': best_auprc_test,
                            ...
                            }, model_save_dir / f'genome_text_learnable_emb_SM_outer_SM_best_AUROC_group_{args.test_group}_ensemble_{ensemble}_fold_{fold}.pth')

                        if outer_test_test_auprc > best_auprc_test:
                            best_auprc_test = outer_test_test_auprc
        ...
        Fold {fold} Outer Test Classification Training Loss: {np.array(outer_cls_train_batch_losses).mean():.4f}, Test Loss: {np.array(outer_cls_test_batch_losses).mean():.4f}
        Fold {fold} Outer Test Classification Training AUROC: {np.array(outer_test_train_auroc).mean():.4f}, Test AUROC: {np.array(outer_test_test_auroc).mean():.4f}, Best: {best_auroc_test:.4f}
        Fold {fold} Outer Test Classification Training AUPRC: {np.array(outer_test_train_auprc).mean():.4f}, Test AUPRC: {np.array(outer_test_test_auprc).mean():.4f}, Best: {best_auprc_test:.4f}
        Fold {fold} Other Classification Training Loss: {np.array(cls_train_batch_losses).mean():.4f}""")
            ...
            ensembled_auroc = roc_auc_score(outer_cls_test_all_labels, ensembled_predictions)
            ensembled_auprc = average_precision_score(outer_cls_test_all_labels, ensembled_predictions)
            ...
            logger.info(f'\nFold {fold} Ensemble AUROC of {group_names[args.test_group]}: {ensembled_auroc:.4f}')
            logger.info(f'Fold {fold} Ensemble AUPRC of {group_names[args.test_group]}: {ensembled_auprc:.4f}')
            # logger.info(f' Ensemble pearson of {group_names[args.test_group]}: {ensembled_pearson:.4f}')

            auroc_list.append(ensembled_auroc)
            auprc_list.append(ensembled_auprc)

        logger.info(f'\n{num_folds} Fold mean Ensembled AUROC of {group_names[args.test_group]}: {np.mean(np.array(auroc_list)):.4f}')
        logger.info(f'\n{num_folds} Fold mean Ensembled AUPRC of {group_names[args.test_group]}: {np.mean(np.array(auprc_list)):.4f}')