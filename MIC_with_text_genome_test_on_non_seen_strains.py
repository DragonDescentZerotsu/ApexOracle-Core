...
model_save_dir = current_directory / 'Checkpoints' / 'genome_text' / 'strain_wize' / f'{num_ensembles}_fold_ensembles'
...
# TODO: 从这里分开 同一个 species 中哪些 strain 作为 train 哪些作为 test
...
# 固定 ATCC ID 的顺序方便分组
for species_name in species_name_ATCC_IDs_map_dict.keys():
    species_name_ATCC_IDs_map_dict[species_name].sort()
...
for i in tqdm(range(len(train_groups)), total=len(train_groups), desc=' Generating train / test spliting for different folds ... '):
    ...
    # 如果到了最后一组，找出那些只有一个 strain 的 species 来作为 test set
    # if i == len(train_groups) - 1:
    #     for species_name, corresponding_ATCC_IDs in species_name_ATCC_IDs_map_dict.items():
    #         # 只有 1 个 strain 的 species 全部放到这里的 test 里专门测试其性能
    #         if len(corresponding_ATCC_IDs) == 1:
    #             test_groups[i].extend(corresponding_ATCC_IDs)
    #         # 其余的直接全部放到 train 里面
    #         else:
    #             train_groups[i].extend(corresponding_ATCC_IDs)
    #     break

    # 只有在不是最后一组的时候才做 fold 的分组
    for species_name, corresponding_ATCC_IDs in species_name_ATCC_IDs_map_dict.items():
        # 只有 1 个 strain 的 species 全部放到这里的训练集里
        if len(corresponding_ATCC_IDs) == 1:
            train_groups[i].extend(corresponding_ATCC_IDs)
        elif len(corresponding_ATCC_IDs) == 2:
            train_groups[i].append(corresponding_ATCC_IDs[i % 2])
            test_groups[i].append(corresponding_ATCC_IDs[(i + 1) % 2])
        else:
            # chunk_length = len(corresponding_ATCC_IDs) // (len(train_groups) - 1)  # 注意这个是真正的 fold 大小
            chunk_length = len(corresponding_ATCC_IDs) // len(train_groups)
            chunked_ATCC_IDs_for_test = corresponding_ATCC_IDs[i*chunk_length : (i+1)*chunk_length]
            chunked_ATCC_IDs_for_train = list(set(corresponding_ATCC_IDs) - set(chunked_ATCC_IDs_for_test))
            train_groups[i].extend(chunked_ATCC_IDs_for_train)
            test_groups[i].extend(chunked_ATCC_IDs_for_test)

group_names = ['fold 1', 'fold 2', 'fold 3', 'fold 4', 'fold 5', 'fold 6']  # 6, 9, 10, 12, 13, 14, 30, 31, 32, 33, 41 缩写字母代表
...
for i, (strain_for_train, strain_for_test, test_group_name) in enumerate(zip(train_groups, test_groups, group_names)):
    ...
    strain_for_test = set(strain_for_test) & all_standard_name_set
    ...
    for ensemble in tqdm(range(num_ensembles), desc=' Doing ensembles '):
        ...
        optimizer = optim.Adam(co_cross_attn_genome.parameters(), lr=1e-5)  # 1e-5
        optimizer.add_param_group({'params': co_cross_attn_text.parameters(), 'lr': 1e-5})
        optimizer.add_param_group({'params': reg_head.parameters(), 'lr': 1e-5})
        optimizer.add_param_group({'params': ChemBERTa_model.parameters(), 'lr': 6e-6})   # TODO: ChemBERTa 和别的学习率不一样
        ...
        for epoch in tqdm(range(num_epochs), desc=f' Training ensemble {ensemble+1}/{num_ensembles} ', leave=False):
            ...
            if epoch == 0:
                with torch.no_grad():
                    ...
                    for batch in tqdm(test_loader, desc=f" Epoch {epoch}/{num_epochs} | evaluating", leave=False):
                        ...
                        for strain_name, label, pred in zip(strain_names, test_batch_labels, test_batch_preds):
                            _speceis_name = ATCC_ID_to_species_name_map_dict[strain_name]
                    ...
                    spearman_correlation = spearmanr(test_all_labels, test_all_preds)[0]
                    pearson_correlation = pearsonr(test_all_labels, test_all_preds)[0]
                    ...
                    for species_name, metrics in r2_MSE_spearman_pearson_species_wise.items():
                        ...
                        print(f'    {species_name}:  {formatted_metrics}')
                    print(
                        f"\n Ensemble {ensemble+1}/{num_ensembles} Epoch {epoch}/{num_epochs}\n Test Loss: {np.array(test_batch_losses).mean():.6f}\n Test R2: {r2:.6f}, Test train mean MIC R2: {r2_train_mean:.6f}\n Test spearman: {spearman_correlation:.6f}\n Test pearson: {pearson_correlation:.6f}")
            ...
            for batch in tqdm(train_loader, desc=f" Ensemble {ensemble+1}/{num_ensembles} Epoch {epoch + 1}/{num_epochs} | training", leave=False):
                input_ids = batch['input_ids'].to(device)
                attention_mask = batch['attention_mask'].to(device)
                labels = batch['label'].to(device)
                padded_genome_embeddings = batch['padded_genome_embeddings']  # .to(torch.float)
            ...
            with torch.no_grad():
                ...
                for batch in tqdm(test_loader, desc=f" Ensemble {ensemble+1}/{num_ensembles} Epoch {epoch + 1}/{num_epochs} | evaluating", leave=False):
                    input_ids = batch['input_ids'].to(device)
                    attention_mask = batch['attention_mask'].to(device)
                    labels = batch['label'].to(device)
                    padded_genome_embeddings = batch['padded_genome_embeddings']  # .to(torch.float)
                ...
                if r2_test > best_R2_test:
                    ...
                    }, model_save_dir / f'genome_text_Strain_wise_best_R2_{args.test_group}_ensemble_{ensemble}.pth')
