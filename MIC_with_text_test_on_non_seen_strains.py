...
if args.parallel and args.test_group is None:
    ...
    exit(1)
genome_embedding_scale_factor = 1
...
embeddings_folder_path = current_directory / 'DataPrepare' / 'Data' / 'Text_Description' / 'ATCC' / 'embeddings'
...
for i, (strain_for_train, strain_for_test, test_group_name) in enumerate(zip(train_groups, test_groups, group_names)):
    ...
    for ensemble in tqdm(range(num_ensembles), desc=' Doing ensembles '):
        ...
        reg_head = RegressionHead(4096, 4096//2, 128, 1, 0.2)
        reg_head.to(device)
        ...
        for epoch in tqdm(range(num_epochs), desc=f' Training ensemble {ensemble+1}/{num_ensembles} ', leave=False):
                ...
                with torch.no_grad():
                    ...
                    for batch in tqdm(test_loader, desc=f" Epoch {epoch}/{num_epochs} | evaluating", leave=False):
                        ...
                        with torch.amp.autocast('cuda', enabled=True):
                            ...
                            mol_cls_embedding = co_cross_attn_genome(mol_cls_embedding, padded_genome_embeddings, 1 - genome_attn_masks)
            ...
            with torch.no_grad():
                ...
                if r2_test > best_R2_test:
                    ...
                    }, current_directory / 'Checkpoints' / 'genome' / '11_fold_ensembles' / f'text_Strain_wise_best_R2_{args.test_group}_ensemble_{ensemble}.pth')
