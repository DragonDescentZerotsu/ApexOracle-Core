...
if __name__=='__main__':
    ...
    model_save_dir = current_folder / 'Checkpoints' / 'genome_text_learnable_emb' / f'strain_wise_synergy' / f'{9}_fold_ensembles'
    ...
    for synergy_data_line in tqdm(synergy_data, desc=' retriving MIC data with only text embeddings '):
    ...
    # dataset = SMILESDataset(data, tokenizer)
        ...
        for ensemble, saved_model_name in tqdm(enumerate(strain_wise_MIC_models), desc=' Doing ensembles '):
            ...
            optimizer.add_param_group({'params': [p for p in reg_head.parameters() if p.requires_grad], 'lr': 1e-5, 'weight_decay': args.weight_decay})
                        ...
                        for gt_batch, t_batch in tqdm(
                            ...
                            if gt_batch is not None:
                                ...
                                with torch.amp.autocast('cuda', enabled=True):
                                    ...
                                    FICI_input = torch.cat((logits[::2], logits[1::2]), dim=1)
                                    logits = FICI_head(FICI_input)
                ...
                for gt_batch, t_batch in tqdm(itertools.zip_longest(gt_train_loader, t_train_loader, fillvalue=None),
                    ...
                    if gt_batch is not None:
                        ...
                        with torch.amp.autocast('cuda', enabled=True):
                            ...
                            mol_cls_embedding = torch.cat((mol_cls_embedding_genome.reshape(-1, 8192), mol_cls_embedding_text.reshape(-1, 4096)), dim=1)
                            logits = reg_head(features=mol_cls_embedding)
                            FICI_input_1 = torch.cat((logits[::2], logits[1::2]), dim=1)
                            FICI_input_2 = torch.cat((logits[1::2], logits[::2]), dim=1)
                            logits_1 = FICI_head(FICI_input_1)
                            logits_2 = FICI_head(FICI_input_2)
                ...
                with torch.no_grad():
                    ...
                    if r2_test > best_R2_test:
                        ...
                        torch.save({
                            ...
                            're_head_state_dict': {k: reg_head.state_dict()[k] for k in [k for k in reg_head.state_dict().keys() if "lora" in k]},
                            ...
                            model_save_dir / f'base_model_{saved_model_name}')
