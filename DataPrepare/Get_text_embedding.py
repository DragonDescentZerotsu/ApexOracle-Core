...
def get_all_text(text_folder_path:Path, text_embed_fold_path:Path):
    text_file_names = set([file.name.split('.')[0] for file in text_folder_path.iterdir()])
    embeded_files = set([f.name.split('.')[0] for f in text_embed_fold_path.iterdir() if f.is_file()])
...
# ATCC_text_dir_path = current_dir / 'Data' / 'Text_Description' / 'ATCC' / 'Text'
ATCC_text_dir_path = Path('/data2/tianang/projects/Ben_ApexOracle_test/DataPrepare/Data/RawText')
# ATCC_text_embed_dir_path = current_dir / 'Data' / 'Text_Description' / 'ATCC' / 'embeddings'
ATCC_text_embed_dir_path = Path('/data2/tianang/projects/Ben_ApexOracle_test/DataPrepare/Data/TextEmb')
