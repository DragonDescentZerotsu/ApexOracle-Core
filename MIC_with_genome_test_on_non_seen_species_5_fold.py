...
all_grouped_species = cluster_species(current_directory/'DataPrepare'/'Data'/'Genome'/'visualization'/'Serratia_replaced_tree.phy', current_directory/'DataPrepare'/'Data'/'Genome'/'old_to_new_NCBI_taxonomy.json', num_clusters=num_clusters)
    ...
    for epoch in tqdm(range(num_epochs), desc=' Training'):
        ...
        if epoch == 0:
            with torch.no_grad():
                ...
                print(
                    f"  Epoch {epoch}/{num_epochs}, Test Loss: {np.array(test_batch_losses).mean():.6f}, Test R2: {r2:.6f}")
