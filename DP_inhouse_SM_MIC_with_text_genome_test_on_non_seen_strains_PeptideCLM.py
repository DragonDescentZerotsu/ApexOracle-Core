...
def get_tokenizer(PepCLM_path: Path):
    vocab_file = PepCLM_path / 'tokenizer'/'new_vocab.txt'
    splits_file = PepCLM_path / 'tokenizer'/'new_splits.txt'
...
model_save_dir = current_directory / 'Checkpoints' / 'genome_text_learnable_emb' / f'strain_wise_w_SM_b_attn' / f'PeptideCLM_fix_{num_ensembles}_fold_ensembles'
...
for i, (strain_for_train, strain_for_test, test_group_name) in enumerate(zip(train_groups, test_groups, group_names)):
    ...
    tokenizer = get_tokenizer(current_directory / 'PeptideCLM')
