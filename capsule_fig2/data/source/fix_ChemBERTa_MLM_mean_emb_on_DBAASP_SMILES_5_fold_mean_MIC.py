...
class ClassificationModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        # 如果有提供 attention_mask，则对有效的 token 进行 mean pooling
        if attention_mask is not None:
            # 将 attention_mask 扩展至与 hidden_state 相同的形状，方便后续计算
            input_mask_expanded = attention_mask.unsqueeze(-1).expand(outputs.last_hidden_state.size()).float()
            # 每个 token 的 embedding 乘以 mask，然后沿序列维度求和
            sum_embeddings = torch.sum(outputs.last_hidden_state * input_mask_expanded, dim=1)
            # 计算有效 token 的数目，防止除以 0
            sum_mask = torch.clamp(input_mask_expanded.sum(dim=1), min=1e-9)
            # 计算 mean pooling 后的句子表示
            mean_embedding = sum_embeddings / sum_mask
        else:
            # 如果没有提供 attention_mask，则直接沿序列维度求平均
            mean_embedding = outputs.last_hidden_state.mean(dim=1)

        # 将平均得到的句子表示传入后续分类器（或其它任务层）
...
if __name__=='__main__':
    ...
    wandb.init(
        ...
        project="Synergy",
        name=f'ChemBERTa_MLM_mena_emb_fix_{num_epochs}epoch_{batch_size}batch size, H100',
