...
# 把前面分开处理的带 * 的手动处理过的 strain 和 不带 * 的有 ATCC 的 strain 重新拼接起来
...
strain_for_test = ['17981']  # 17981  '11060', '29930'
...
# 去掉 train_data 中在 test_data 中出现过的所有 AMP 的数据
...
test_AMP_IDs = set(test_data[:, 0])
indices = np.where(~np.isin(train_data[:, 0], list(test_AMP_IDs)))[0]
...
for epoch in tqdm(range(num_epochs), desc=' Training'):
    ...
    with torch.no_grad():
        ...
        print(f"  Epoch {epoch + 1}/{num_epochs}\n Training Loss: {np.array(train_batch_losses).mean()}, Test Loss: {np.array(test_batch_losses).mean()}\n Test R2: {r2_test:.6f}, Training R2: {r2_train:.6f}")