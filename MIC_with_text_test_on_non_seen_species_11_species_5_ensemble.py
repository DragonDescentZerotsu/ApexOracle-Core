...
model_save_dir = current_directory / 'Checkpoints' / 'text' / '11_species' / f'{num_ensembles}_fold_ensembles'
...
for i, (grouped_species, grouped_strains, test_group_name) in enumerate(zip(all_grouped_species, all_grouped_strains, group_names)):
    ...
    for ensemble in tqdm(range(num_ensembles), desc=' Doing ensembles '):
        ...
        optimizer.add_param_group({'params': ChemBERTa_model.parameters(), 'lr': 1e-6, 'weight_decay': args.weight_decay})   # TODO: ChemBERTa 和别的学习率不一样
        ...
        for epoch in tqdm(range(num_epochs), desc=f' Training ensemble {ensemble+1}/{num_ensembles} ', leave=False):
            ...
            with torch.no_grad():
                ...
                if r2_test > best_R2_test:
                    ...
                    }, model_save_dir / f'text_11_Species_wise_best_R2_{args.test_group}_ensemble_{ensemble}.pth')
