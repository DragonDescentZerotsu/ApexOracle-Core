...
model_save_dir = current_directory / 'Checkpoints' / 'genome_text_learnable_emb' / f'strain_wise_w_SM_b_attn' / f'MolFormer_{num_ensembles}_fold_ensembles'
...
for i, (strain_for_train, strain_for_test, test_group_name) in enumerate(zip(train_groups, test_groups, group_names)):
    ...
    model_name = "ibm/MoLFormer-XL-both-10pct" # "ibm/MoLFormer-XL-both-10pct" / "DeepChem/ChemBERTa-77M-MTR"
    ...
    for ensemble in tqdm(range(num_ensembles), desc=' Doing ensembles '):
        ...
        ChemBERTa_model = AutoModel.from_pretrained(model_name, trust_remote_code=True)
