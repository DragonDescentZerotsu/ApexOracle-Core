...
if __name__=='__main__':
    ...
    train_data_path = current_folder / 'DataPrepare' / 'Data' / 'APEX_train.csv'  # 替换为你的数据路径
    train_data = pd.read_csv(train_data_path)
    test_data_path = current_folder / 'DataPrepare' / 'Data' / 'APEX_test.csv'  # 替换为你的数据路径
    ...
    wandb.init(
        ...
        project="Synergy",
        name=f'ChemBERTa_all_data_{num_epochs}epoch_{batch_size}batch size, L40S, inhouse data',
    ...
    # 加载模型
    model = ClassificationModel(model_name, num_labels=11)
    model.to(device)
    ...
    r2_tracker = R2Tracker(num_tasks=11)
    best_R2_score = 0.0
    ...
    for epoch in range(num_epochs):
        ...
        if R2_mean > best_R2_score:
            ...
            }, current_folder / 'Checkpoints' / f'inhouse_data_best.pth')
            # torch.save(model.state_dict(), f'./compare_APEX/checkpoint/APEX_model_fold_{fold + 1}.pt')
        wandb.log({"loss": loss.item(), "R2_mean": R2_mean, "epoch": epoch})
    ...
    torch.save({
        ...
        'ChemBERTa_state_dict': model.bert.state_dict(),
        're_head_state_dict': model.classifier.state_dict()
    }, current_folder / 'Checkpoints' / f'inhouse_data_final.pth')
