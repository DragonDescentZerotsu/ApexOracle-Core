    ...
    for epoch in tqdm(range(num_epochs), desc=' Training'):
        ...
        if epoch == 0:
            with torch.no_grad():
                ...
                print(
                    f"  Epoch {epoch}/{num_epochs}, Test Loss: {np.array(test_batch_losses).mean():.6f}, Test R2: {r2:.6f}, Test train mean MIC R2: {r2_train_mean:.6f}")
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(test_loader, desc=f" Epoch {epoch + 1}/{num_epochs} | evaluating", leave=False):
                input_ids = batch['input_ids'].to(device)
                attention_mask = batch['attention_mask'].to(device)
                labels = batch['label'].to(device)
                padded_genome_embeddings = batch['padded_genome_embeddings']  # .to(torch.float)
            ...
            print(f"  Epoch {epoch + 1}/{num_epochs}, Training Loss: {np.array(train_batch_losses).mean()}, Test Loss: {np.array(test_batch_losses).mean()}, Test R2: {r2_test:.6f}, Training R2: {r2_train:.6f}, Best Test R2: {best_R2_test:.6f}")