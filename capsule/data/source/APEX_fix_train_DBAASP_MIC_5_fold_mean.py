...
wandb.init(
    ...
    project="Synergy",
    name=f'APEX_fix_DBAASP_AASeq_data_{num_epochs}_epochs_{batch_size}_batch_size_5_fold_mean, more param, best',
...
for fold, (train_idx, test_idx) in enumerate(kf.split(dataset)): # TODO
    ...
    # pretrained_path = '/data2/tianang/projects/Synergy/compare_APEX/APEX_ckpt/APEX_pretrained_encoder_state_dict.ckpt'
    pretrained_path = '/data2/tianang/projects/Synergy/compare_APEX/APEX_ckpt/APEX_pretrained_encoder_state_dict_best.ckpt'
    APEX_pretrained_state_dict = torch.load(pretrained_path)
    # model = AMP_model_fix(emb, emb_size, num_rnn_layers = 2, dim_h = 256)  # TODO
    model = AMP_model_fix(emb, emb_size, num_rnn_layers=3, dim_h=128)  # TODO
    model.load_state_dict(APEX_pretrained_state_dict)
    print(f' APEX encoder param: {sum(p.numel() for p in model.parameters())}')
    # model.apply(init_weights)
    model.to(device)
    for param in model.parameters():
        param.requires_grad = False

    cls_head = RegressionHead(128, 512, 256, 19)
    cls_head.to(device)

    # 定义损失函数和优化器
    criterion = MultiTaskLoss()
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, cls_head.parameters()), lr=1e-4)
