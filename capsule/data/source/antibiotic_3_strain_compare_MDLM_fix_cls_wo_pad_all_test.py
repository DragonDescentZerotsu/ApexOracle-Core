...
if __name__ == '__main__':
    ...
    model_save_dir = current_directory / 'Checkpoints' / 'genome_text_learnable_emb' / 'antibiotic_3_strain_compare' / f'MDLM_fix_cls_sm_all_test_{num_ensembles}_fold_ensembles'
    ...
    for group_i, test_group_name in enumerate(group_names):
        ...
        for ensemble in tqdm(range(num_ensembles), desc=f' Doing ensembles'):
            # 设置对应的 随机数种子
            ...
            if gt_flag:
                ...
                outer_test_SM_test_loader = DataLoader(outer_test_SM_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn_cls)
            else:
                ...
                outer_test_SM_test_loader = DataLoader(outer_test_SM_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn_text_only_cls)
            ...
            for epoch in tqdm(range(num_epochs), desc=f' Training ensemble {ensemble+1}/{num_ensembles}', leave=False):
                ...
                with torch.no_grad():
                    ...
                    if outer_test_test_auroc > best_auroc_test:
                        ...
                        }, model_save_dir / f'genome_text_learnable_emb_SM_outer_SM_best_AUROC_group_{args.test_group}_ensemble_{ensemble}.pth')
    ...
    Outer Test Classification Test Loss: {np.array(outer_cls_test_batch_losses).mean():.4f}
    Outer Test Classification Test AUROC: {np.array(outer_test_test_auroc).mean():.4f}, Best: {best_auroc_test:.4f}
    Outer Test Classification Test AUPRC: {np.array(outer_test_test_auprc).mean():.4f}, Best: {best_auprc_test:.4f}
    Other Classification Training Loss: {np.array(cls_train_batch_losses).mean():.4f}
    ...
    Train pearson:{pearson_train:.4f}
      ...
      Text onlyTrain pearson:{t_pearson_train:.4f}""")
        ...
        logger.info(f'\n Ensemble AUROC of {group_names[args.test_group]}: {ensembled_auroc:.4f}')
        logger.info(f' Ensemble AUPRC of {group_names[args.test_group]}: {ensembled_auprc:.4f}')
