...
class Convolution(torch.nn.Module):
    def __init__(self, irreps_in, irreps_sh, irreps_out, num_neighbors) -> None:
        ...
        tp = FullyConnectedTensorProduct(
            ...
            internal_weights=False,  # FullyConnected 这里却没有 internal_weights，之后 forward 的时候要把 weight 输进去
            shared_weights=False,
        )
        self.fc = FullyConnectedNet([3, 256, tp.weight_numel], torch.relu)  # 这个 3 对应距离标量 embedding 到 3 个 basis 函数上
        self.tp = tp
        self.irreps_out = self.tp.irreps_out

    def forward(self, node_features, edge_src, edge_dst, edge_attr, edge_scalars) -> torch.Tensor:

        # edge_scalars 就是 edge_length_embedded , 应该是每一个分量都算作 scalar
        weight = self.fc(edge_scalars)

        # node_features[edge_src] 数量和边的数量一样，weight的数量也是，所以是每个 node 和 edge 的 TensorProduct 都对应一个 weight
        # 每一个 weight 都是来自于 edge 长度的编码，所以这里就把 length of edge 编进网络里了
...
class Network(torch.nn.Module):
    ...
    def forward(self, data) -> torch.Tensor:
        ...
        edge_attr = o3.spherical_harmonics(l=self.irreps_sh, x=edge_vec, normalize=True, normalization="component")  # 边向量 embedding 到球谐函数
        edge_length_embedded = (

            # 其实就是在 start 到 end 上等距离取 numbers 个基函数然后算每一个 f_i(x) 作为 embedding 罢了
            soft_one_hot_linspace(x=edge_vec.norm(dim=1), start=0.5, end=2.5, number=3, basis="smooth_finite", cutoff=True)
            * 3**0.5
        )

        # x 是 conv 层的 node_features
