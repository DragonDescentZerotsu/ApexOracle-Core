...
parser.add_argument(
    '-m', '--num_ensembles',  # 可选参数
    type=int,
    default=5,
    help='How many ensembles to train'
)
...
num_ensembles = args.num_ensembles  # 要集成几个 model 来做预测
...
model_save_dir = current_directory / 'Checkpoints' / 'genome' / '3_species' / f'{num_ensembles}_fold_ensembles'
        ...
        for epoch in tqdm(range(num_epochs), desc=f' Training ensemble {ensemble+1}/{num_ensembles} ', leave=False):
                ...
                with torch.no_grad():
                    ...
                    for batch in tqdm(test_loader, desc=f" Epoch {epoch}/{num_epochs} | evaluating", leave=False):
                        ...
                        with torch.amp.autocast('cuda', enabled=True):
                            ...
                            mol_cls_embedding = co_cross_attn_genome(mol_cls_embedding, padded_genome_embeddings,
                                                                     1 - genome_attn_masks)
            ...
            with torch.no_grad():
                ...
                if r2_test > best_R2_test:
                    ...
                    }, model_save_dir / f'best_R2_{args.test_group}_ensemble_{ensemble}.pth')
