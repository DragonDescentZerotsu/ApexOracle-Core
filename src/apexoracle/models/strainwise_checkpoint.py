"""Strict loader for final strain-wise paper checkpoints."""
...
@dataclass
class StrainwiseCheckpointComponents:
    ...
    def eval(self) -> "StrainwiseCheckpointComponents":
    ...
    def to(self, device: torch.device) -> "StrainwiseCheckpointComponents":
...
def inspect_checkpoint_contract(checkpoint: dict) -> dict:
    ...
    if missing or unexpected:
        raise ValueError(
            f"strain-wise checkpoint contract mismatch: missing={sorted(missing)}, "
...
def load_legacy_strainwise_checkpoint(
    path: Path,
    *,
    ...
    mmap: bool = True,
) -> tuple[StrainwiseCheckpointComponents, dict]:
    ...
    components = StrainwiseCheckpointComponents(
...
def predict_genome_text(
    ...
    fused = torch.cat((genome.reshape(-1, genome.shape[-1]), text.reshape(-1, text.shape[-1])), dim=1)
