...
if __name__=='__main__':
    ...
    wandb.init(
        ...
        project="Synergy",
        name=f'Synergy, LoRA, LoRA_r = {lora_r}, L40S, DBAASP only, simple',
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(synergy_dataset)):
        ...
        cls_head = RegressionHead(ChemBERTa_model.config.hidden_size * 2, num_targets=3)
        cls_head.to(device)
        ...
        for epoch in range(num_epochs):
            ...
            for batch in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs} | training"):
                ...
                # merged_emd = merge_layer(two_mol_concat_emb)[:, None, :]  # [B/2, 1, D]
                ...
                # concat_cls_emd = torch.cat((cls_embedding_1, merged_cls_emd, cls_embedding_2), dim=1)

                # logits = cls_head(concat_cls_emd)
                logits = cls_head(two_mol_concat_emb)
