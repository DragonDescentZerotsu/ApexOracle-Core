...
class CompareDataset(Dataset):
    ...
    def exclude_non_test_data(self, test_DBAASP_ids, outpu_test_compare_pairs = False):
        """
                测试集中的数据最好是也不能参与 RL ，这里都剔除掉
                :param outpu_test_compare_pairs: 是否要输出被去掉的与test有关的数据是哪些
                :param test_DBAASP_ids: 所有在测试集中的 DBAASP id
        ...
        # 反转哪些 行 是要去掉的
        pair_line_indexs_to_remove = set(range(len(self.input_ids))) - pair_line_indexs_to_remove
        # 这个是用于返回测试 test 上分类正确率的数据, 已经tokenize好的
        # self.input_ids = np.array(self.input_ids, dtype=object)
        test_compare_pairs_input_ids = np.array(self.input_ids, dtype=object)[np.sort(np.array(list(pair_line_indexs_to_remove)))]
        # test_compare_pairs_input_ids = self.input_ids[np.sort(np.array(pair_line_indexs_to_remove))]
        ...
        print(
            f' removing overlaping data, which is {len(pair_line_indexs_to_remove) / len(self.reordered_DBAASP_id) * 100}% of the original compare data')
        # self.reordered_smiles = np.delete(self.reordered_smiles, list(pair_line_indexs_to_remove))
        self.more_attention_masks = [mask for idx, mask in enumerate(self.more_attention_masks) if idx not in pair_line_indexs_to_remove]
        self.attention_mask = [mask for idx, mask in enumerate(self.attention_mask) if idx not in pair_line_indexs_to_remove]
        self.input_ids = [input_id for idx, input_id in enumerate(self.input_ids) if idx not in pair_line_indexs_to_remove]
    ...
    def update_test_compare_data(self, ChemBERTa_model, re_head, batch_size, test_DBAASP_ids):
        ...
        while batch * batch_size < len(paired_input_ids):
            ...
            with torch.no_grad():
                ...
                psudo_mean_MIC = torch.sum(logits * mean_weight, dim=1)
                pred_labels = ((psudo_mean_MIC[::2] - psudo_mean_MIC[1::2]) < 0).to(dtype=torch.int)
                mean_MIC = torch.sum(torch.exp(-logits) * 10 * mean_weight, dim=1)  # 得到真的 mean MIC
                all_pred_mean_MICs.append(mean_MIC.detach().cpu().numpy())
                all_pred_psudo_mean_MICs.append(psudo_mean_MIC.detach().cpu().numpy())
        ...
        all_pred_mean_MICs = np.concatenate(all_pred_mean_MICs).reshape(-1)  # 注意这里得到的是 原始 MIC
        all_pred_psudo_mean_MICs = np.concatenate(all_pred_psudo_mean_MICs).reshape(-1)
        all_pred_labels = np.concatenate(all_pred_labels).reshape(-1)
        acc = np.sum(((all_pred_psudo_mean_MICs[::2]-all_pred_psudo_mean_MICs[1::2])<0).astype(int)) / (len(all_pred_mean_MICs)/2)

        right,wrong = 0,0
        for input_ids_1, input_ids_2, attn_mask_1, attn_mask_2, more_attn_mask_1, more_attn_mask_2, mean_MIC_1, mean_MIC_2, psudo_mean_MIC_1, psudo_mean_MIC_2, DBAASP_id1, DBAASP_id2 in zip(paired_input_ids[::2], paired_input_ids[1::2], paired_input_attn_masks[::2], paired_input_attn_masks[1::2], test_compare_pairs_more_attn_masks[::2], test_compare_pairs_more_attn_masks[1::2], all_pred_mean_MICs[::2], all_pred_mean_MICs[1::2], all_pred_psudo_mean_MICs[::2], all_pred_psudo_mean_MICs[1::2], test_compare_pairs_DBAASP_ids[::2], test_compare_pairs_DBAASP_ids[1::2]):
            if mean_MIC_1 > mean_MIC_2 * 3 and psudo_mean_MIC_1 < psudo_mean_MIC_2:
                ...
                right += 1
            elif mean_MIC_2 > mean_MIC_1 * 3 and psudo_mean_MIC_1 > psudo_mean_MIC_2:
...
def assess_test_classification_acc(ChemBERTa_model, re_head, paired_input_ids, paired_input_attn_masks, batch_size, pad_token_id):
    ...
    while batch*batch_size < len(paired_input_ids):
        ...
        with torch.no_grad():
            ...
            label_mask = (torch.max(paired_mean_MIC, dim=1)[0] / torch.min(paired_mean_MIC, dim=1)[0] > 1.7).to(dtype=torch.int)
...
if __name__=='__main__':
    ...
    wandb.init(
        ...
        project="Synergy",
        name=f'ChemBERTa_all_data_{num_epochs}epoch_{batch_size}batch size, L40S, compare only reversed PsudoMIC, SSL 0.5 fold change, 3 fold predict, only test, auto fold change',
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(dataset)):
        ...
        compare_dataset = CompareDataset(
            pd.read_csv(current_folder / 'DataPrepare' / 'Data' / 'DBAASP_id_SMILES_compare_cleaned_w_mean_MIC_50.csv'),
            tokenizer)
        ...
        checkpoint = torch.load(current_folder / 'Checkpoints' / f'no_RL_best_R2_fold{fold}.pth', map_location=device)
        ChemBERTa_model.load_state_dict(checkpoint['ChemBERTa_state_dict'])
        re_head.load_state_dict(checkpoint['re_head_state_dict'])

        compare_dataset.exclude_non_test_data(test_subset_DBAASP_id)
        compare_dataset.update_test_compare_data(ChemBERTa_model, re_head, compare_batch_size, test_subset_DBAASP_id)

        # test_compare_pairs_input_ids, test_compare_pairs_attn_masks, test_compare_pairs_more_attn_masks, test_compare_pairs_DBAASP_ids = compare_dataset.exclude_test_data(test_subset_DBAASP_id, outpu_test_compare_pairs=True)
        # 重置模型
        re_head = RegressionHead(ChemBERTa_model.config.hidden_size, num_targets=19)
        re_head.to(device)
        ChemBERTa_model = AutoModel.from_pretrained(model_name)
