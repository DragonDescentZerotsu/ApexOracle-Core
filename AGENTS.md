...
- **正在运行的实验：** 7-model × 5-fold 正式共享数据实验于 2026-07-17 在四张 H100 上启动，输出目录为 `results/fig2b_shared_original_protocol/`。每个 fold 保持原始 200 epochs、batch size 200、Adam、learning rate `1e-4`、模型特有 head、frozen backbone 的训练模式以及 held-out-fold checkpoint selection。加速只包括 fold/GPU 并行和一次性缓存 held-out fold 上确定性的 frozen-backbone `eval()` feature；训练阶段的 backbone dropout 仍逐 batch 开启，APEX held-out head dropout 也保持原行为。
