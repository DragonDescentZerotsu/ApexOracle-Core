...
| Fig. 1b fine-tuned ApexOracle | 同一入口的 `--mode fine-tune --fold N`；checkpoint：`MDLM_fix_cls_10_fold_ensembles` | **代码路径已统一 / 完整 ensemble 补实验进行中。** 合并本机与 node002 后恢复 104/150 个历史 checkpoint；作者于 2026-07-20 确认最终比较必须使用每折 10-member ensemble，单成员 pooled OOF 只保留为 sensitivity。RN4220 fold 4 member 0 已补训，剩余 45 个缺失成员正在本机 3 张 H100 与 node002 8 张 A100 上并行补齐，输出与历史目录隔离。`--mode molecule-only` 对应旧 `wo_SAND`。 |
