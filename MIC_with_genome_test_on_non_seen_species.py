...
def exclude_wrong_species_ATCC_map(Evo_MIC_data_with_genome_embedding:np.array, genome_ID_to_species_first_name_dict):
    ...
    print(f'\n original data length {original_length}\n cleaned data length {len(cleaned_data)}\n')
...
for epoch in tqdm(range(num_epochs), desc=' Training'):
    ...
    if epoch == 0:
        with torch.no_grad():
            ...
            for batch in tqdm(test_loader, desc=f" Epoch {epoch + 1}/{num_epochs} | evaluating", leave=False):
                ...
                test_all_labels.extend(labels.squeeze().detach().cpu().tolist())
                test_all_preds.extend(logits.squeeze().detach().cpu().tolist())
    ...
    for batch in tqdm(train_loader, desc=f" Epoch {epoch + 1}/{num_epochs} | training", leave=False):
        ...
        train_all_labels.extend(labels.squeeze().detach().cpu().tolist())
        train_all_preds.extend(logits.squeeze().detach().cpu().tolist())
    ...
    with torch.no_grad():
        ...
        print(f"  Epoch {epoch + 1}/{num_epochs}, Training Loss: {np.array(train_batch_losses).mean()}, Test Loss: {np.array(test_batch_losses).mean()}, Test R2: {r2_test:.6f}, Training R2: {r2_train:.6f}")